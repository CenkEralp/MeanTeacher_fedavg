...
class Server(object):
    ...
    def setup(self, **init_kwargs):
        ...
        init_net(self.teacher_model, **self.init_config)
    ...
    def transmit_model(self, sampled_client_indices=None):
        ...
        if sampled_client_indices is None:
            ...
            for idx in tqdm(sampled_client_indices, leave=False):
                ...
                self.clients[idx].teacher_model = copy.deepcopy(self.teacher_model)
    ...
    def average_model(self, sampled_client_indices, coefficients):
        ...
        averaged_weights = OrderedDict()
        for it, idx in tqdm(enumerate(sampled_client_indices), leave=False):
            local_weights = self.clients[idx].teacher_model.state_dict()
            for key in self.teacher_model.state_dict().keys():
                if it == 0:
                    averaged_weights[key] = coefficients[it] * local_weights[key]
                else:
                    averaged_weights[key] += coefficients[it] * local_weights[key]
        self.teacher_model.load_state_dict(averaged_weights)
    ...
    def fit(self):
        ...
        for r in range(self.num_rounds):
            ...
            self.writer.add_scalars(
                'Loss',
                {f"[{self.dataset_name}] C_{self.fraction}, E_{self.local_epochs}, B_{self.batch_size}, IID_{self.iid}": test_loss},
                self._round
                )
            self.writer.add_scalars(
                'Accuracy', 
                {f"[{self.dataset_name}] C_{self.fraction}, E_{self.local_epochs}, B_{self.batch_size}, IID_{self.iid}": test_accuracy},
                self._round
                )
